		...
		new File(outDirectory).mkdirs();
			...
			if (reference.equals("v37")) {
				...
				if (!new File(reference).exists()) {
					System.out.println("Download Reference GRCh37: "
							+ GRCh37_PATH + ".gz");
					FileUtils.copyURLToFile(new URL(GRCh37_PATH + ".gz"),
							new File(reference + ".gz"));
					FileUtils.copyURLToFile(new URL(GRCh37_PATH + ".fai"),
			...
			if (genome.endsWith("zip")) {
				FileUtil.unzip(genome);
				genome = genome.replace(".zip", ".txt");
			}
			...
			while (genomeReader.next()) {
				...
				if (line.getGenotype().contains("I") || genotype.contains("D")
						|| genotype.contains("-") || genotype.length() > 2) {
					continue;
				}

				// check exclude list
				if (excludeList != null
						&& Arrays.asList(excludeList.split(",")).contains(
								line.getChromosome())) {
					continue;
				}
				...
				if (splitFiles && !chromosome.equals(prev)) {
					...
					String name = genome.substring(
							genome.lastIndexOf(File.separator) + 1,
							genome.lastIndexOf("."));
					...
					VariantContextWriterBuilder builder = new VariantContextWriterBuilder()
							.setOutputFile(
									outDirectory + File.separator + name
											+ ".vcf.gz").unsetOption(
				...
				String refPos = refFasta.getSubsequenceAt(line.getChromosome(),
						line.getPos(), line.getPos()).getBaseString();
				...
				if (g1 != null && !g0.equals(g1)) {
					altAllele = Allele.create(!refPos.equals(g0) ? g0 : g1,
							false);
					heterozygous = true;
				}
				...
				if (heterozygous) {
					...
					vcfWriter.add(createVC(header, chromosome, line.getRsid(),
							alleles, alleles, line.getPos()));
