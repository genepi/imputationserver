...
import genepi.minicloudmac.hadoop.imputation.sort.ChunkKey;
import genepi.minicloudmac.hadoop.imputation.sort.ChunkValue;
import genepi.minicloudmac.hadoop.util.ParameterStore;
import genepi.minicloudmac.hadoop.validation.io.bed.BedUtil;
import genepi.minicloudmac.hadoop.validation.io.vcf.VcfChunk;
import genepi.minicloudmac.hadoop.validation.io.vcf.VcfChunkOutput;
...
public class MiniMacVcfMapper extends
	...
	protected void setup(Context context) throws IOException,
		...
		pattern = parameters.get(MiniMacVcfJob.REF_PANEL_PATTERN);
		output = parameters.get(MiniMacVcfJob.OUTPUT);
		window = parameters.getInteger(MiniMacVcfJob.WINDOW, 0);
		phasing = parameters.get(MiniMacVcfJob.PHASING);
		String hdfsPath = parameters.get(MiniMacVcfJob.REF_PANEL_HDFS);
